/**
 * Connection String Input Component with connection testing
 */

import { useState } from 'react';
import { CheckCircle2, XCircle, Loader2, Plug, Eye, EyeOff } from 'lucide-react';
...
interface ConnectionStringInputProps {
  ...
  description?: string;
  required?: boolean;
  testConnection?: (connectionString: string, databaseType: string) => Promise<boolean>;
  databaseType?: string; // Required for testing
  maskInput?: boolean; // Whether to mask the input (default: false for connection strings)
}
...
export function ConnectionStringInput({
  ...
  const handleConnect = async () => {
    ...
    try {
      const connected = await testConnection(value.trim(), databaseType);
      setIsConnected(connected);
      onConnectionChange?.(connected);
      
      if (connected) {
        toast.success(`${databaseType.toUpperCase()} connection successful!`);
      } else {
        toast.error(`${databaseType.toUpperCase()} connection failed. Please check your connection string.`);
      }
    } catch (error: any) {
      setIsConnected(false);
      onConnectionChange?.(false);
      toast.error(error.message || `${databaseType.toUpperCase()} connection failed`);
    } finally {
  ...
  const handleValueChange = (newValue: string) => {
    onChange(newValue);
    // Reset connection status when value changes
    if (isConnected !== null) {
      setIsConnected(null);
      onConnectionChange?.(false);
    }
    ...
    <div className="space-y-2">
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          {/* Show/Hide toggle button (only if maskInput is true) */}
          {maskInput && (
            <button
              type="button"
      ...
      {isConnected === true && (
        <p className="text-xs text-green-400 flex items-center gap-1">
          ...
          {databaseType.toUpperCase()} connection verified and saved
        </p>
      )}
      {isConnected === false && (
        <p className="text-xs text-red-400 flex items-center gap-1">
          <XCircle className="w-3 h-3" />
          Connection failed. Please check your connection string and try again.
        </p>
      )}
    </div>
  );
